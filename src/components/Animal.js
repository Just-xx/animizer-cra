...
const Animal = ({ animal }) => {
  ...
  return (
    ...
    >
      <ClipLoader color="#fff" loading={!isFetched && !error} />
      {error && <div className={styles.fetchError}>Unable to get the image <br/> :(</div>}
      {isFetched && (
        <>
          {!isVideo ? (
          ...
          <div className={styles.btns}>
            <Button asHref to={imgUrl}>
              Download
            </Button>
            <Button asBtn onClick={newImg}>
              Next
            </Button>
          </div>
        </>
